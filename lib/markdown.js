 ...
 **/
exports.parse = function( source, dialect ) {
 ...
 **/
exports.toHTML = function toHTML( source , dialect , options ) {
  var input = exports.toHTMLTree( source , dialect , options );

  return exports.renderJsonML( input );
};
 ...
 *  [[parse]].
 **/
exports.toHTMLTree = function toHTMLTree( input, dialect , options ) {
...
// Instiki dialect
Markdown.dialects.Instiki = Markdown.subclassDialect( Markdown.dialects.Maruku );

Markdown.dialects.Instiki.block.code_backticks = function code( block, next ) {
  var ret = [],
      //re = /^(?: {0,3}\t| {4})(.*)\n?/,
      re_begin = /^(?:```([^\n\s`]+)?)\s*/,
      re_end = /^\n?```\s*$/;
...
Markdown.dialects.Instiki.inline[ "[[" ] = function wiki_link( text ) {
  var m = text.match( /^\[\[(.+?)(?:\|(.+))?\]\]/ );
...
for ( var dialect in Markdown.dialects ) {
  if ( Markdown.dialects[dialect].block !== undefined ) {
    Markdown.buildBlockOrder( Markdown.dialects[dialect].block );
  }

  if ( Markdown.dialects[dialect].inline !== undefined ) {
    Markdown.buildInlinePatterns( Markdown.dialects[dialect].inline );
  }
}
